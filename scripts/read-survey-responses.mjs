...
console.table(
  rows.map((row) => ({
    ...
    locale: row.locale,
    hasManagedCare: row.hasManagedCare ?? "",
    careTarget: row.careTarget ?? "",
    caredPersonAge: row.caredPersonAge ?? "",
    familyCaregivers: row.familyCaregivers ?? "",
    coordinationFrequency: row.coordinationFrequency ?? "",
    organizationMethod: row.organizationMethod,
    organizationAppName: row.organizationAppName ?? "",
    careChallenges: row.careChallenges.join(", "),
    hadIncident: row.hadIncident,
    currentlyPaysCare: row.currentlyPaysCare ?? "",
    carePayer: row.carePayer ?? "",
    appTrust: row.appTrust,
    appInterest: row.appInterest,
    whatsappPreference: row.whatsappPreference ?? "",
